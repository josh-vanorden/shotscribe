...
- `scripts/fit-icon.swift` + `scripts/make-iconset.sh` — the app icon from `assets/ShotScribe-artwork.png` (fit to Apple's grid, then iconset + icns).
- `scripts/render-pane.swift`, `scripts/svg-to-png.swift`, `scripts/make-brand-art.swift` — off-screen renders, brand marks.
...
## What this repo can invoke — read on 2026-09-24

### Executable tracked files

`scripts/make-iconset.sh`, `scripts/package-app.sh`
